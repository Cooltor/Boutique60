...
$content .= "<a href=\"index.php?categorie=$categorie[categorie]\">$categorie[categorie]</a>"; // Je passe dans l'url la catégorie du produit
$content .= "</li>";
$content .= "</ul>";
...
while($produit = $produits->fetch(PDO::FETCH_ASSOC)){
    ...
    $content .= "<p class=\"card-text\">$detail ...<a href=\"fiche_produit.php?id_produit=$produit[id_produit]\" class=\"text-decoration-none\">Lire la suite</a></p>";
    $content .= "<p><a href=\"\" class=\"card-link fw-bold text-decoration-none text-dark\">$produit[prix] €</p></a>";
    $content .= "<a href=\"fiche_produit.php?id_produit=$produit[id_produit]\" class=\"btn btn-dark\">Voir ce produit</a>";
    $content .= '</div>';
    $content .= '</div>';
    $content .= '</div>';
